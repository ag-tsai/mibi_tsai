...
{var selects=tsai.tiles_selects('tiles_build', -1);
 var b='<table id="tile_builder">\n <tr>\n  <td>Tile name</td><td>Section ID</td><td>FOV (&micro;m)</td><td>Raster size</td><td>Preset</td><td>&nbsp</td></tr>'
  +'\n <tr>'
  +'\n  <td><input id="tiles_build_name" type="text" value=""/></td>'
...
{var style=' style="background-color:'+tsai.canvas.line_colors[tile%tsai.canvas.line_colors.length][0]+'; color:'+tsai.canvas.line_colors[tile%tsai.canvas.line_colors.length][1]+'"';
 ...
 var b='\n<div id="tile_'+tile+'" class="tile_form tile_collapsed"'
  ...
  +'\n   </td>'
  +'\n   <td><input   name="action_radio" id="tile_'+tile+'_tma"       type="radio" onclick="tsai.tma_builder('    +tile+');"'+(tsai.action.item==tile && tsai.action.type=='tma'      ?' checked':'')+                          '/><label for="tile_'+tile+'_tma"> TMA</label></td>'
...
{var scan=(!isNaN(tsai.coordinates.sed[0][0]) && !(typeof tsai.coordinates.sed[0][0]=='string')
 ...
 +' {var sed='
  ...
  +   ' var canvas=document.createElement(\'canvas\');'
  +   ' var context=canvas.getContext(\'2d\');'
  +   ' var width=Math.round(fov_pixels_x*columns);'
  +   ' var height=Math.round(fov_pixels_y*rows);'
  +   ' canvas.style=\'display:none;\';'
  +   ' canvas.width=width;'
  ...
  +    'var file_coordinates=\'_(\'+corners.join(\',\')+\').png\';'
  +   ' canvas.toBlob('
  +    ' (blob)=>'
  +    ' {var url=window.URL || window.webkitURL;'
  +     ' var anchor=document.createElement(\'a\');'
  ...
  +     ' document.body.removeChild(anchor);'
  +    '});'
  +    'console.log(\'Finished full SED: \'+(columns*rows)+\' FOVs, \'+fov_microns_x+\'×\'+fov_microns_y+\' μm at \'+(Math.round(fov_pixels_x*100)/100)+\'×\'+fov_pixels_y+\', \'+delay+\' ms delay\\n\\n\');'
  +   ' this.scanning=false;'
  +   '},'
  ...
  +    ' case \'KeyC\': if(event.shiftKey) this.check(false); break;'
  +    ' case \'KeyN\': if(event.shiftKey) this.check(true); break;'
  ...
  +    '\\nShift+C: Check corrections (checkerboard)'
  +    '\\nShift+N: Check corrections (3×3)'
  +    '\\nShift+M: Set dwell time (ms)'
...
{// determine top left of dragged rectangle and calculate first tile centroid
 ...
 document.getElementById('tile_'+tile_new+'_name').value='';
...
navigation_code_arrows(clear)
{if(typeof clear==undefined) clear='';
 else
 {clear=parseInt(clear);
  if(clear==0 || isNaN(clear)) clear='';
  else clear=' this.context.clearRect(0,0,'+clear+','+clear+');';
 }
